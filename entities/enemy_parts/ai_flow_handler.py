# --- START OF FILE ai_flow_handler.py ---
...
class AIFlowMixin:
    def update_flow_selection(self, player):
        # Если идет анимация атаки, движения или КД, прерываем выбор
        ...
        # Единый объединенный пул кандидатов атак
        candidates = []
        any_satisfied = False
        ...
        for f_idx, flow in enumerate(self.flows):
            ...
            if flow["_cooldown_timer"] == 0:
                if self.check_player_in_flow_zone(player.rect, f_idx):
                    ...
                    # Шанс масштабируется пропорционально прогрессу накопления таймера
        ...
        # 2. Сбор кандидатов из Sequences (кроме тех, которые используются внутри Flow)
        for seq_idx, seq in enumerate(self.sequences):
            is_in_flow = any(
                seq_idx == step.get("seq_idx") 
        ...
        # Если ни одна зона в пуле еще полностью не накопила время удержания, ИИ ждет
        if not any_satisfied:
            return

        # Роллим из единого пула пропорционально высчитанным весам
        if candidates:
            ...
            if total_weight > 0:
                ...
                # Инкрементируем счетчик последовательных атак
                self.consecutive_triggers[key] = self.consecutive_triggers.get(key, 0) + 1
                
                if self.consecutive_triggers[key] >= limit:
                    # Лимит исчерпан: обнуляем счетчик и сбрасываем удержание, заставляя копить время заново
                    self.consecutive_triggers[key] = 0
                    self.trigger_hold_timers[key] = 0
                else:
                    # Если накопление сохраняется, то принудительно оставляем его заряженным во время атаки
                    if accumulate:
                        req_hold = trig_zone.get("hold_time", 0)
