                ...
                for p in act["phases"]:
                    if p.get("delay", 0) == act["timeline_timer"] - 1:
                        if p.get("direction") == "teleport":
                            # Мгновенная телепортация относительно центра врага
                            ...
                            # Физическое выталкивание для предотвращения застревания в стенах/платформах
                            ...
                            for plat in game.platforms:
                                if self.rect.colliderect(plat.rect):
                                    self.rect.x, self.rect.y = old_x, old_y # Возврат при застревании
                                    break
                        ...
                        if not act["hitbox_damaged_flags"][s_idx]:
                            ...
                            if is_colliding and c_timer == -1:
                                if is_parry_successful:
                                    self._resolve_hitbox_parry(player, s_data, s_idx, act, selected_attack)
                                else:
                                ...
                                if is_parry_successful:
                                    ...
                                    if act["hitbox_contact_timers"][s_idx] == 0:
                                        self._resolve_hitbox_damage(player, s_data, s_idx, act, selected_attack)
                                        
        return movement_vx_bonus, movement_vy_bonus, movement_applied_any

    def _start_action_from_step(self, step, game):
        if step["type"] == "attack":
            ...
            if atk_idx < len(self.attacks):
                ...
                shapes = selected_attack.get("shapes", [])
            ...
            if m_idx < len(self.movements):
                ...
                # Исключаем 15-кадровый застой. Фаза телепорта имеет мгновенный вклад в 1 кадр.
                durations = []
                for p in phases:
            ...
            if game is not None:
                ...
                # Обработка шаблонов снарядов врага (или legacy-fallback на свойства шага)
                ...
                # Логика AIM AT PLAYER: расчет вектора на игрока в момент спавна
