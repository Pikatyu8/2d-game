            ...
            if self.state == "post_action":
                ...
                if self.state == "chase":
                    if self.movement_type == "flying":
                        ...
                        # Преследуем, только если игрок находится ВНЕ триггерной зоны остановки
                        if not self.check_player_in_stop_zone(player.rect):
                            if distance > 0:
                        ...
                        # Двигаемся, только если игрок ВНЕ триггерной зоны остановки
                        if self.check_player_in_stop_zone(player.rect):
                            vx = 0
                        else:
                            vx = self.speed * 1.4 * self.direction
                else:  # Patrol
