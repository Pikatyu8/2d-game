...
class EnemyRenderMixin:
    ...
    def draw(self, surface, show_debug, player_rect, camera_x=0, camera_y=0, zoom=1.0, editing_trigger_idx=None, editing_attack_idx=None, editing_box_idx=None, inspector_tab=None, selected_move_idx=None):
        ...
        if self.is_winding_up:
            ...
            if is_flash_on:
                ...
                head_color = (255, int(200 * (1.0 - progress)), 0)
            else:
        ...
        font_name = pygame.font.SysFont(None, font_name_size)
        ...
        if self.parry_accumulated_damage > 0:
            ...
            font_parry = pygame.font.SysFont(None, font_size, bold=True)
        ...
        if self.is_winding_up:
            windup_max = getattr(self, "attack_windup_max", 35)
            progress = max(0.0, min(1.0, (windup_max - self.attack_windup_timer) / windup_max))
            warn_color = (255, int(220 * (1.0 - progress)), 0)
            ...
            if int(getattr(self, "windup_phase", 0.0)) % 2 == 0:
                font_warning = pygame.font.SysFont(None, 20, bold=True)
                ...
                if delay <= self.attack_timeline_timer <= (delay + duration):
                    ...
                    if vis_box:
                        ...
                        if stype == "rectangle":
                            ...
                            v_cy = self.rect.centery + oy # Исправлено v_oy -> oy для предотвращения вылета
        ...
        # Отрисовка линии и жирной точки траектории телепортации
        # А) Gameplay-теграф: Отрисовывается во время боя, до момента кадра телепортации
                ...
                for p in act.get("phases", []):
                    if p.get("direction") == "teleport":
                        ...
                        if act.get("timeline_timer", 0) <= delay:
                            ...
                            line_color = (180, 80, 255)  # Насыщенный пурпурный
        ...
        # Б) Editor-теграф: Показывает точку телепорта в Enemy Editor, когда выбран соответствующий шаблон в UI
        if inspector_tab == "DETAILED_MOVEMENT" and selected_move_idx is not None and selected_move_idx < len(self.movements):
            ...
            for p in m_template.get("phases", []):
                if p.get("direction") == "teleport":
                    ...
                    # Защита от фейкового игрока в редакторе (Fallback на разворот)
                    is_real_player = (player_rect is not None and player_rect.width > 0 and player_rect.x >= 0)
                    if is_real_player:
                        dir_to_player = 1 if player_rect.centerx >= start_cx else -1
                    else:
                    ...
                    line_color = (255, 100, 255)  # Ярко-розовый для редактора
                    pygame.draw.line(surface, line_color, (int(sx1), int(sy1)), (int(sx2), int(sy2)), max(1, int(2 * zoom)))
                    pygame.draw.circle(surface, line_color, (int(sx2), int(sy2)), max(4, int(7 * zoom)))
                    pygame.draw.circle(surface, (255, 255, 255), (int(sx2), int(sy2)), max(1, int(3 * zoom)))
        ...
        if show_debug:
            ...
            if is_stop_visible and stop_dist > 0:
                ...
                font_stop = pygame.font.SysFont(None, font_size)
            ...
            for z_idx, zone in enumerate(self.trigger_zones):
                ...
                if atk_zone_data:
                    ...
                    if editing_trigger_idx is not None and z_idx != editing_trigger_idx:
                        ...
                        target_draw_surface = alpha_surf
                    else:
                    ...
                    # Текст отладки: Hold накопление и счетчик атак до сброса
                    font_size = max(8, int(11 * zoom))
                    font_hold = pygame.font.SysFont(None, font_size)
            ...
            for f_idx, flow in enumerate(self.flows):
                ...
                if flow_zone_data:
                    ...
                    font_size = max(8, int(11 * zoom))
                    font_flow = pygame.font.SysFont(None, font_size)
            ...
            for a_idx, att in enumerate(self.attacks):
                ...
                for s_idx, (s_type, val) in enumerate(shapes_data):
                    ...
                    if is_active_edit:
                        ...
                        font_att = pygame.font.SysFont(None, font_size)
