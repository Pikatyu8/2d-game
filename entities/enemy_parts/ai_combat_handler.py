...
class AICombatMixin:
    def _resolve_hitbox_parry(self, player, s_data, s_idx, act, selected_attack, game=None):
        ...
        # Применяем полученную силу отдачи к игроку
        ...
        # Воспроизведение звука и запуск хитстопа
        if game:
            game.hitstop_timer = 3
            game.play_sound("parry")

    def _resolve_hitbox_damage(self, player, s_data, s_idx, act, selected_attack, game=None):
        dmg = s_data.get("damage", selected_attack.get("damage", 1))
        ...
        player.take_damage(dmg, force_x, force_y, game)
