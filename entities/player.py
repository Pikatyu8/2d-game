...
class Player:
    def __init__(self, config):
        ...
        self.vx = 0
        self.vy = 0
        self.knockback_vx = 0  # Горизонтальная сила отдачи
    ...
    def handle_input(self):
        ...
        # Отключаем управление во время хитстана (получения урона), чтобы отдачу нельзя было прервать ходьбой
    ...
    def take_damage(self, amount, force_x, force_y=None, game=None):
        if self.damage_flash_timer == 0 and self.parry_success_flash_timer == 0:
            ...
            self.knockback_vx = force_x  # Применяем горизонтальную отдачу
            
            # Применяем вертикальную отдачу
    ...
    def update(self, platforms, enemies, game=None):
        ...
        # Плавное угасание горизонтальной отдачи игрока
        ...
        # Складываем скорость ходьбы и текущую скорость отдачи
        ...
        if self.is_attacking:
            ...
            for enemy in enemies:
                if enemy not in self.hit_enemies and self.attack_rect.colliderect(enemy.rect):
                    ...
                    # Придаем отдачу врагу при попадании по нему (по направлению взгляда игрока)
