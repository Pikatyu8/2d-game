...
class RightPanelAttacksTabMixin:
    ...
    def draw_enemy_detailed_projectile_tab(self, target_enemy_raw, y_offset, mouse_clicked_this_frame, mouse_pos):
        ...
        y_offset += 25
        cd_minus, cd_plus = self.draw_property_row("Shoot CD", curr_proj.setdefault("shoot_cooldown", 30), y_offset)
        y_offset += 35
        ...
        if mouse_clicked_this_frame:
            ...
            if cd_minus.collidepoint(mouse_pos): curr_proj["shoot_cooldown"] = max(0, curr_proj.setdefault("shoot_cooldown", 30) - 5)
            if cd_plus.collidepoint(mouse_pos):  curr_proj["shoot_cooldown"] = curr_proj.setdefault("shoot_cooldown", 30) + 5
