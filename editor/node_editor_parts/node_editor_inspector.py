...
# Вспомогательные хелперы для безопасного приведения типов перед отправкой в Dear PyGui
...
class NodeEditorInspectorMixin:
    ...
    def rebuild_inspector(self, node_id):
        ...
        if node.type == "Root":
            ...
            dpg.add_input_int(label="Stop Distance", default_value=safe_int(node.properties.get("stop_dist"), 60), callback=self.update_property, user_data=(node_id, "stop_dist"), parent="properties_panel")
